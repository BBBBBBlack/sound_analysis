...
class HMMMLP():
    ...
    def forward_dnn(self, pi, a, o, digit):
        ...
        log_alpha[0] += np.array(
            [mlp_ll[0][self.uniq_state_dict[(digit, j)]] + self.log_prior[self.uniq_state_dict[(digit, j)]]
             for j in range(J)])

        for t in range(1, T):
            for j in range(J):
                mlp_ll_t = mlp_ll[t][self.uniq_state_dict[(digit, j)]] + self.log_prior[
...
if __name__ == '__main__':
    ...
    parser.add_argument('train', type=str, help='training data', default="train_1digit.feat")
    parser.add_argument('test', type=str, help='test data',default="tem")
