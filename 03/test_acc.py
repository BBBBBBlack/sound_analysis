...
def test_acc(modelpath, evaldatadir, spk_indexlist_path, result_txt_path):
    ...
    model.load_state_dict(torch.load(modelpath), strict=False)  ## 模型 加载上 你训练过的模型的参数
    ...
    # 创建混淆矩阵图
    plt.figure()
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=['bird', 'cat', 'dog', 'tiger'],
                yticklabels=['bird', 'cat', 'dog', 'tiger'])
...
def test():
    ...
    ## 模型文件的路径
    model_path = "e2\\model_checkpoints\\epoch_000299.pth"

    ## 测试语音文件夹 路径
    eval_datadir = "test_set"
